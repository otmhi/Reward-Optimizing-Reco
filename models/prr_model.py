...
class PRRModel(tf.Module):
    def __init__(self, nb_bidding_features:int, embedding_dim:int, nb_gs_categories:int,
        ...
        if not type(bidding_init) == type(None):
            self.bidding_weight = tf.Variable(bidding_init, trainable=False, dtype=tf.float32)
        else:
        ...
        if not type(add_position_bias_init) == type(None):
            self.add_position_bias = tf.Variable(add_position_bias_init, trainable=False, dtype=tf.float32)
        else:
            self.add_position_bias = tf.Variable(xavier_init([1, max_slate_size]),
                                     trainable=True)
        if not type(mult_position_bias_init) == type(None):
            self.mult_position_bias = tf.Variable(mult_position_bias_init, trainable=False, dtype=tf.float32)
        else:
    ...
    def train(self, dataset, num_epochs:int, batch_size:int, criterion, optimizer, **kwargs):
        dataloader = Dataloader(dataset, batch_size)
        
        for _ in tqdm(range(num_epochs)):
            epoch_loss = []
            for i, batch in enumerate(dataloader()):
                loss, _ = train_step(self, criterion, optimizer, batch)
